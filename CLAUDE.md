...
- **Media library page** (`/library`, "没干正事 / Off Duty"): watched films/TV and played games as a poster grid. Data is a committed snapshot in `src/data/library/` regenerated manually by `node scripts/library/build-snapshot.mjs` from the owner's local douban-export folder (never wired into the site build); `--covers` thumbnails cover art to R2 under `library/`. That script is the only gate deciding what library data goes public: books, wish lists, backlogs, and owner-held entries stay out.
